...
全部操作集中在單一 Tkinter GUI。最大的賣點是**同網域多工作站分散運算**：只要在主控端 `~/.ssh/config` 為每台工作站設好別名，GUI 就能把干涉對同時派給所有機器平行處理，計算時間大致隨健康節點數成比例縮短。唯一的硬前提是共享儲存——**每台工作站都必須把同一個 NAS 掛載在相同路徑**（例如 `/mnt/SARDB/snap2mintpy/project`），因為所有中間與最終成果都寫在那裡。詳見「叢集設定」一章。另有一組命令列工具，涵蓋叢集監控、GNSS 交叉驗證、角反射器分析，以及以 ASF HyP3 雲端運算取代本機 SNAP 的替代管線。

![速度場範例](docs/figs/velocity_example.png)

*範例場景：台灣南投縣魚池鄉頭社盆地——俗稱「活盆地」。這裡原是遠古湖泊，數千年水生植物腐化沉積形成深達數十公尺的草泥炭土層。泥炭土像海綿，有水膨脹、缺水下陷：當地農業由水稻改為旱作，排水口又為防洪而拓寬，地下水位急遽下降，泥炭土隨之壓密、乾縮與氧化（台語俗稱「ㄏㄨ去」），曾使當地農莊因地基不穩整棟傾斜。此過程幾乎不可逆——泥炭土一旦乾涸便失去吸水能力、體積無法回復；下陷又使盆地高程更低、排洪更難、淹水風險更高，形成惡性循環。上圖速度場即捕捉到這個過程：下陷集中於泥炭土分布區、最大約 20 mm/yr，並與同址 GNSS 站（TSB2）交叉驗證一致。*
...
主控端裝好後，要讓新機器（`worker01`）分擔運算：

1. **把同一個 NAS 掛載在與主控端相同的路徑**——硬性要求，所有中間與最終成果都寫在共享儲存上。worker 的 `/etc/fstab` 範例：

   ```
   //nas.local/SARDB  /mnt/SARDB  cifs  credentials=/home/your_user/.smbcred,uid=1000,iocharset=utf8  0  0
   ```

   repo 本身建議放在 NAS 上（例如 `/mnt/SARDB/snap2mintpy/`）：每個節點自動看到同一份程式碼與專案目錄，不需要逐台複製。

2. **在 worker 上安裝執行環境**（步驟同主控端的「安裝」一章）：
   - ESA SNAP（讓 `gpt` 可執行）與 SNAPHU
   - Miniconda ＋ 含 MintPy 與必要 Python 套件的環境
   - 若 `gpt` 不在登入 shell 的 `$PATH`，在 worker 的 `~/FastISCE.config` 裡 export——主控端的遠端指令啟動 worker 前會先 source 這個檔。

3. **在主控端設定免密 SSH** 並為 worker 取別名：

   ```bash
   ssh-keygen -t ed25519                  # 沒有金鑰時做一次即可
...
5. **在 GUI 勾選該主機**——Tab 2 會自動列出 `~/.ssh/config` 裡所有 `Host` 別名。勾選 `worker01`、開啟叢集模式執行；用逐機 log 分頁或 `cluster_progress.py` 監看。

每加一台就重複以上步驟——勾選的健康節點越多，同時處理的干涉對越多。
